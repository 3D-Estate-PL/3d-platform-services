...
public class Endpoint : Endpoint<CopyTourStyleRequest, CopyTourStyleConfigurationResponse>
{
    ...
    {
        Post($"/{Paths.TourStyleConfigurations}/{{ConfigurationId}}/{Paths.Place}/{Paths.Styles}/{{ConfigurationStyleId}}/clone");
        AllowAnonymous();
        Description(b => b
                .Accepts<CopyTourStyleRequest>("application/json")
                .Produces<CopyTourStyleConfigurationResponse>(200, "application/json"),
            true);
        Summary(s =>
        {
            ...
            s.Description = "Copy tour style";
            s.ExampleRequest = new CopyTourStyleRequest
            {
    ...
    public override async Task HandleAsync(CopyTourStyleRequest request, CancellationToken c)
    {
        ...
        var clonedTourStyleConfiguration = tourStyleConfiguration.Clone($"{place.Styles.Count()+1}",request.CustomName);
        place.Styles.Add(clonedTourStyleConfiguration);
        
        await _tourStyleConfigurationRepository.UpsertConfigurationAsync(configuration);


        var response = new CopyTourStyleConfigurationResponse
        {
            Id  = configuration.Id,
